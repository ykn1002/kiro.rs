...
- **桌面版 App（推荐）** — 双击即用：托盘常驻、免登录管理面板、内置 AWS SSO 一键导入（自动扫描本机缓存），全程无需终端。图文步骤见 **[桌面版新手引导](onboarding/桌面版新手引导.md)**。
- **命令行二进制** — 解压压缩包在终端启动，适合服务器 / 无桌面环境。**本文以下部分即命令行流程。**

---

> 以下面向**命令行二进制**：假设你拿到的是打好的压缩包，里面**已经包含配置文件 `config.json`**，默认管理密码为 `admin`，无需手写配置。将带你走完：**下载并启动 → 登录管理界面 → 用 AWS SSO 导入凭证 → 修改接入 Key → 用 CC Switch 接入 Claude Code**。
