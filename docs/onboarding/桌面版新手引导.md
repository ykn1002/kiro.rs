...
![凭据管理页，右上角的 AWS SSO 导入按钮](https://cdn.jsdelivr.net/gh/ykn1002/kiro.rs@master/docs/onboarding/img/01-credentials-empty.png)
...
![AWS SSO 导入对话框，顶部为一键导入区块](https://cdn.jsdelivr.net/gh/ykn1002/kiro.rs@master/docs/onboarding/img/02-sso-dialog.png)
...
![一键导入完成，显示「新增 0，已存在 1，失败 0」](https://cdn.jsdelivr.net/gh/ykn1002/kiro.rs@master/docs/onboarding/img/03-scan-result.png)
...
![实时监控页，可用凭据与调用统计](https://cdn.jsdelivr.net/gh/ykn1002/kiro.rs@master/docs/onboarding/img/04-monitor.png)
...
![系统设置 → 常规 → 客户端 API Key](https://cdn.jsdelivr.net/gh/ykn1002/kiro.rs@master/docs/onboarding/img/05-settings-apikey.png)
...
![系统设置 → 桌面 Tab：开机启动、静默启动、自动轻量模式](https://cdn.jsdelivr.net/gh/ykn1002/kiro.rs@master/docs/onboarding/img/06-settings-desktop.png)
...
同一个 Tab 里还有「自动轻量模式」：关窗后销毁界面的渲染进程来省内存，能**有效降低**后台占用，下次唤出时重新加载页面、约 1 秒延迟；下面的「进入延迟」控制关窗后多久进入（分钟，0 表示关窗立即释放）。**这只影响界面进程，代理服务本身一直在后台跑，不影响 Claude Code / Codex 的请求。** 内存敏感就开着，想要唤出零延迟就关掉。
