...
- **第三方 API 透传凭证** — 可把标准协议的第三方 API（Claude / Codex）作为透传后端混入同一凭据池，与 Kiro 凭证一起负载均衡与故障转移、按入站协议隔离：Kiro 凭证走翻译链，透传凭证原样转发原始字节，额度耗尽（`403 INSUFFICIENT_BALANCE`）自动切换，余额走上游 `/v1/usage` 查询（USD）。
